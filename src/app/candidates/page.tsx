                      ...
                      <td>
                        <div className="user-cell" style={{ alignItems: 'flex-start' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            {/* PANEL DE EVALUACIÓN PSICOMÉTRICA DIRECTO EN BANDEJA */}
                            <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '1px dashed #cbd5e1', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                              <span style={{ fontSize: '11px', fontWeight: '800', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.02em' }}>Evaluación Psicométrica:</span>
                              {(() => {
                                const psychTest = psychometricTests.find(t => t.resume_id === r.id);
                                if (!psychTest) {
                                  return (
                                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                      <span className="status-badge status-pending" style={{ fontSize: '10px', padding: '3px 8px' }}>
                                        ⏳ PENDIENTE
                                      </span>
                                      <button 
                                        className="track-btn" 
                                        style={{ padding: '3px 8px', fontSize: '11px', color: '#2563eb', borderColor: '#dbeafe', height: '24px' }}
                                        onClick={() => handleSendPsychometricEmail(r, r.position || 'Candidato')}
                                        disabled={sendingPsychometricId === r.id}
                                      >
                                        {sendingPsychometricId === r.id ? '...' : '✉️ Enviar'}
                                      </button>
                                      <button 
                                        className="track-btn" 
                                        style={{ padding: '3px 8px', fontSize: '11px', color: '#8b5cf6', borderColor: '#ddd6fe', height: '24px' }}
                                        onClick={() => {
                                          const protocol = window.location.protocol;
                                          const host = window.location.host;
                                          setQrModalUrl(`${protocol}//${host}/evaluacion/${r.id}`);
                                        }}
                                      >
                                        📱 QR
                                      </button>
                                ...
                                return (
                                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                    <span 
                                      className={`status-badge ${isCompleted ? 'status-synced' : 'status-pending'}`}
                                      style={{ 
                                        fontSize: '10px',
                                        padding: '3px 8px'
                                      }}
                                    ...
                                    {isCompleted ? (
                                      <button 
                                        className="track-btn" 
                                        style={{ padding: '3px 8px', fontSize: '11px', color: '#10b981', borderColor: '#bbf7d0', height: '24px' }}
                                        onClick={() => setViewingPsychometric({ test: psychTest, candidate: r })}
                                      >
                                        📊 Resultados
                                      </button>
                                    ) : (
                                      <>
                                        <button 
                                          ...
                                          disabled={sendingPsychometricId === r.id}
                                        >
                                          {sendingPsychometricId === r.id ? '...' : '✉️ Reenviar'}
                                        </button>
                                        <button 
                                          className="track-btn" 
                                          style={{ padding: '3px 8px', fontSize: '11px', color: '#8b5cf6', borderColor: '#ddd6fe', height: '24px' }}
